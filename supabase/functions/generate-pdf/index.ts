...
// Edge function generate-pdf deployed
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (isDetailedOrder && !isThermal) {
      ...
      const clientDocStr = clientDoc ? ` - ${clientDoc}` : ''
      drawText(`Cliente: ${client?.CODIGO || 0} - ${clientName}${clientDocStr}`, leftColX, y, {
        size: 10,
        font: fontBold,
      })
      ...
      if (!isThermal) {
        ...
        // Resumo de Acertos Thermal - Removido
      }
    }
      ...
      const docString = clientDoc ? ` - ${clientDoc}` : ''
      ...
      const infoSize = 9
      drawText(`Cliente: ${clientCode} - ${clientName}${docString}`, margins.left, y, {
        size: infoSize,
        font: fontBold,
        maxWidth: width - 20,
      })
      ...
      if (history && history.length > 0) {
        ...
        for (const h of history) {
          ...
          const isAjusteForma = (h.forma || h.FORMA || h.forma_pagamento || '').toLowerCase().includes('ajuste');
          if (h.isAjuste || isAjusteForma) {
             continue;
          }
          ...
          const debColor = valDeb > 0 ? rgb(1, 0, 0) : rgb(0, 0, 0)
          drawText(`A pagar (debito): ${formatCurrency(valDeb)}`, width - margins.right, y, { size: 8, align: 'right', font: fontBold, color: debColor })
          y -= 10

          const medColor = valMed > 0 ? rgb(0, 0, 1) : rgb(0, 0, 0)
          drawText(`Media Mensal: ${formatCurrency(valMed)}`, margins.left, y, { size: 8, color: medColor })
          y -= 15 
        }
        drawLine(y)
      }
