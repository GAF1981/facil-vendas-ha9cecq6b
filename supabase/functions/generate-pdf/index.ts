...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (isDetailedOrder && !isThermal) {
      ...
      drawText(
        `Cliente: ${client?.CODIGO || 0} - ${clientName}${clientDocStr}`,
        leftColX,
        y,
        {
          size: 10,
      ...
      const infoSize = 9
      drawText(
        `Cliente: ${clientCode} - ${clientName}${docString}`,
        margins.left,
        y,
        {
          size: infoSize,
      ...
      if (history && history.length > 0) {
        ...
        for (const h of history) {
          ...
          const isAjusteForma = (h.forma || h.FORMA || h.forma_pagamento || '')
            .toLowerCase()
            .includes('ajuste')
          ...
          drawText(
            ...
            width - margins.right,
            y,
            { size: 8, align: 'right', font: fontBold, color: debColor },
          )
          y -= 10
