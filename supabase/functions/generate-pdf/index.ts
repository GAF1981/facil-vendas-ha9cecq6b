...
const calculateThermalHeight = (body: any) => {
  ...
  if (
    ...
    // Removed settlements rendering height for closing reports per user request
  ...
  try {
    ...
    if (isDetailedOrder && !isThermal) {
      ...
      if (!isThermal) {
        ...
        // Resumo de Acertos A4 - Removido a pedido do usuário
      } else {
        drawText('FACIL VENDAS', width / 2, y, {
          size: 14,
          font: fontBold,
        ...
        // Resumo de Acertos Thermal - Removido a pedido do usuário
      }
    } else if (
      isThermal &&
