...
export const rotaService = {
  ...
  async applyPrimeiroAcertoRoutine(
    activeRotaId: number,
    ...
    for (let i = 0; i < orderIds.length; i += 1000) {
      ...
      const { data: orders } = await supabase
        .from('BANCO_DE_DADOS')
        .select(
          'NÚMERO DO PEDIDO, TIPO, DATA DO ACERTO, CODIGO FUNCIONARIO, CÓDIGO DO CLIENTE',
        )
    ...
    for (const client of clients) {
      ...
      if (order.TIPO === 'CAPTAÇÃO' || order.TIPO === 'Captação') {
        const dataAcerto = order['DATA DO ACERTO']
          ? new Date(order['DATA DO ACERTO'])
          : null
